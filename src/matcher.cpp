...
Matcher::Matcher(const std::vector<std::string> &lines)
	: lines(lines),
	lower_lines(lines_to_lower(lines)) {
	reset_matches();
}

std::vector<std::string> Matcher::lines_to_lower(const std::vector<std::string> &lines) {
...
bool Matcher::fuzzy_match(const std::string &str, const std::vector<std::string> &tokens) {
